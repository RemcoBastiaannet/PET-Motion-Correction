...
def move_Phantom(motion, nFrames, trueShiftAmplitude, trueSlope, trueSquareSlope, image, stationary): 
    ...
    for iFrame in range(nFrames): 
        if 'Sine' in motion:
            ...
            if (phaseMod >= math.pi/2.0 and phaseMod < 3.0*math.pi/2.0): shiftX = trueSlope*sur + trueSquareSlope*sur**2
            else: shiftX = trueSlope*sur - trueSquareSlope*sur**2 + 2*trueSquareSlope*trueShiftAmplitude**2   
            if ((not stationary) and (iFrame > nFrames/2)): 
                shift += 2*trueShiftAmplitude
        elif 'Step' in motion: 
            sur = iFrame*trueShiftAmplitude
            shift = trueSlope*sur
