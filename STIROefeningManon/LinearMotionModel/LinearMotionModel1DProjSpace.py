...
for i in range(1): 
    ...
    for offset in range(0,trueShiftPixels/2+1,1): 
        projectionPList = []
    ...
    if (min(quadErrorSums) != previousOffset): 
        for i in range(len(quadErrorSumList)): 
            if(quadErrorSumList[i]['quadErrorSum'] == min(quadErrorSums)): 
                offsetFound = quadErrorSumList[i]['offset'] - previousOffset
                previousOffset += offsetFound
                offsetFoundList.append(offsetFound)


    #_________________________MOTION COMPENSATION_______________________________
    recon1S = stir.FloatVoxelsOnCartesianGrid(projdata_info, 1,
                        stir.FloatCartesianCoordinate3D(stir.make_FloatCoordinate(0,0,0)),
                        stir.IntCartesianCoordinate3D(stir.make_IntCoordinate(np.shape(originalImageP)[0],np.shape(originalImageP)[1],np.shape(originalImageP)[2] )))  
    recon1S.fill(1) # moet er staan

    MotionModel.setOffset(-offsetFound)
    recon1 = stir.OSMAPOSLReconstruction3DFloat(projmatrix, 'config_Proj_MC_1.par')
    recon1.set_up(recon1S)
    recon1.reconstruct(recon1S)

    recon1P = stirextra.to_numpy(recon1S)

    recon2S = stir.FloatVoxelsOnCartesianGrid(projdata_info, 1,
                        stir.FloatCartesianCoordinate3D(stir.make_FloatCoordinate(0,0,0)),
                        stir.IntCartesianCoordinate3D(stir.make_IntCoordinate(np.shape(originalImageP)[0],np.shape(originalImageP)[1],np.shape(originalImageP)[2] )))  
    recon2S.fill(1) # moet er staan

    MotionModel.setOffset(+offsetFound)
    recon2 = stir.OSMAPOSLReconstruction3DFloat(projmatrix, 'config_Proj_MC_@.par')
    recon2.set_up(recon2S)
    recon2.reconstruct(recon2S)

    recon2P = stirextra.to_numpy(recon2S)
    
    #_________________________UPDATE GUESS_______________________________
    guessP = 0.5*(recon1P + recon2P) 
    plt.imshow(guessP[0,:,:]), plt.title('Motion corrected image'), plt.show()

print 'Offsets:', offsetFoundList 